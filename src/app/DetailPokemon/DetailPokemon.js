...
import {Button, CircularProgress, Grid, IconButton, ListItemText, TextField, Typography} from "@material-ui/core"
                ...
                _.isEmpty(detailState) ? (
                    ...
                    <Grid container>
                            ...
                            <Grid container>
                                ...
                                <Grid item xs={6}>
                                    {gotIt !== 1 && <IconButton onClick={handleCatchIt}>
                                        <img src={process.env.PUBLIC_URL + '/assets/img/pokeball.png'} width={25}
                                             alt={"pokeball"}/>
                                    ...
                                    {detailState?.types?.map((type, idx) => {
                                        return (
                                            <Grid item xs={4} key={idx}>
                                                <ListItemText
                                                    className={[classes.roundedBorder, classes.whiteFontColor, classes[type.type.name]].join(" ")}
                        ...
                        {detailState?.abilities?.map((ability, idx) => {
                            return (
                                <Grid item className={[classes.center_grid, classes.roundedBorder].join(" ")} xs={6}
                                      key={idx}>
